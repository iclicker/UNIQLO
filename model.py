...
batch_size = img_loader.BATCH_SIZE
...
def input():
  ...
  """
  images, labels = img_loader.input()
  return images, labels

def input_eval():
    images, labels = img_loader.input_eval()
    return images, labels

def input_test():
    images, labels, num_files = img_loader.input_test()
    return images, labels, num_files

def inference(images, keep_prob3, keep_prob4):
  ...
  with tf.variable_scope('drop3') as scope:
    drop3 = tf.nn.dropout(local3, keep_prob3)
    _activation_summary(drop3)

  # local4
  with tf.variable_scope('local4') as scope:
    weights = _variable_with_weight_decay('weights', shape=[1000, 240],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [240], tf.constant_initializer(0.1))
    local4 = tf.nn.relu(tf.matmul(drop3, weights) + biases, name=scope.name)
    _activation_summary(local4)
  with tf.variable_scope('drop4') as scope:
    drop4 = tf.nn.dropout(local4, keep_prob4)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [240, NUM_CLASSES],
                                          stddev=1/240.0, wd=0.0)
